...
// Constant-time check of the organizer password (gates sensitive actions like
// regenerate/add/delete/undo). Always false when none is configured, so a locked
// action fails closed rather than silently opening.
function checkOrganizer(input) {
  if (!config.organizerPassword) return false;
  const a = Buffer.from(String(input || ''), 'utf8');
  const b = Buffer.from(config.organizerPassword, 'utf8');
  if (a.length !== b.length) return false;
  return crypto.timingSafeEqual(a, b);
}
...
module.exports = { COOKIE, issue, isValid, checkPassword, checkOrganizer, setCookie, clearCookie, requireStaff };

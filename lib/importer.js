...
// Header labels our own export adds (see lib/exporter.js). Skipped when a sheet is
// re-imported so they never become bogus per-attendee "extra" fields.
const SYSTEM_HEADERS = new Set([
  'added manually', 'source', 'status', 'entered at (ist)', 'registered at (ist)',
]);
  ...
  try {
    ...
    for (let r = 2; r <= ws.rowCount; r++) {
      ...
      for (const [idx, label] of headers) {
        ...
        if (SYSTEM_HEADERS.has(label.toLowerCase())) continue; // don't re-import our export's own columns
      ...
      if (email) {
        ...
        if (existing.rows[0]) {
          await client.query(`UPDATE attendees SET name = $1, extra = $2, source = 'import' WHERE id = $3`,
            [name, extraJson, existing.rows[0].id]);
          updated++;
        } else {
          await client.query(`INSERT INTO attendees (name, email, extra, token, source) VALUES ($1, $2, $3, $4, 'import')`,

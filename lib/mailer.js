...
function html(name, token) {
  ...
  const c = config;
  // "View on Google Maps" pill button under the venue (omitted if no map URL is
  // set). Table + inline-block anchor so the marker icon and label stay
  // vertically centered and the whole pill is clickable across email clients.
  // The marker PNG has a white background (no alpha), so the pill is white too
  // and the icon blends in; served like the QR from <BASE_URL>/img/.
  const mapLink = c.eventMapUrl
    ? `<br/>`
      + `<table role="presentation" cellpadding="0" cellspacing="0" style="margin-top:12px;border-collapse:separate;">`
      + `<tr><td style="background:#ffffff;border:1px solid #e4002b;border-radius:6px;">`
      + `<a href="${escapeHtml(c.eventMapUrl)}" style="display:inline-block;padding:8px 15px;font-size:13px;font-weight:bold;color:#e4002b;text-decoration:none;letter-spacing:0.5px;line-height:1;">`
      + `<img src="${c.baseUrl}/img/map-marker.png" alt="" width="19" height="18" style="vertical-align:middle;border:0;outline:none;"/>`
      + `<span style="vertical-align:middle;">&nbsp;&nbsp;View location on Google Maps&nbsp;&nbsp;&rarr;</span>`
      + `</a></td></tr></table>`
    : '';
  const P = [];
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#fbe9ec;border-left:3px solid #e4002b;border-radius:4px;"><tr>
<td style="padding:12px 16px;font-size:14px;color:#1a1a1a;line-height:1.5;"><strong style="color:#e4002b;">Please carry your ID cards</strong> &mdash; bring your college ID card and a photo ID card for verification at the entrance.</td>
</tr></table></td></tr>
...
function text(name) {
  ...
  return `TRACES OF LENSES x FUJIFILM — Interactive Photography Workshop\n\n`
    ...
    + `Please carry your ID cards — bring your college ID card and a photo ID card for verification at the entrance.\n\n`
    + `Date:  ${c.eventDate}\n`
    + `Time:  ${c.eventTime}\n`
    + `Venue: ${c.eventVenue}\n`
...
//   ids      — array of attendee ids: target exactly these (that have an email),
//              regardless of whether they were emailed before; overrides resend
//   onProgress({ sent, total, name }) — optional callback per successful send
async function sendEmails({ resend = false, dryRun = false, ids = null, onProgress } = {}) {
  const select = Array.isArray(ids);
  const clean = select ? ids.map((n) => parseInt(n, 10)).filter(Number.isInteger) : [];
  // An explicit-but-empty selection means "nobody" — don't fall through to a
  // bulk send.
  if (select && clean.length === 0) {
    return dryRun
      ? { dryRun: true, total: 0, sent: 0, failed: 0, recipients: [] }
  ...
  if (select) {
    params.push(clean);
    where = `email IS NOT NULL AND email <> '' AND id = ANY($1::bigint[])`;
  } else if (resend) {
    where = `email IS NOT NULL AND email <> ''`;
  } else {
    where = `email IS NOT NULL AND email <> '' AND email_sent_at IS NULL`;
  }
  const { rows } = await query(
    `SELECT id, name, email, token FROM attendees WHERE ${where} ORDER BY id`, params);
  ...
  for (const a of rows) {
    try {
      ...
      // Stamp the send time; flag as a resend if they'd already been emailed
      // (the RHS sees the pre-UPDATE email_sent_at, so this is correct in one shot).
      await query(
        `UPDATE attendees
           SET resent = resent OR (email_sent_at IS NOT NULL),
               email_sent_at = now()
         WHERE id = $1`,
        [a.id]
      );
